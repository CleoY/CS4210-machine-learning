...
#Using Keras to Load the Dataset. Every image is represented as a 28×28 array
    #rather than a 1D array of size 784. Moreover, the pixel intensities are represented
    #as integers (from 0 to 255) rather than floats (from 0.0 to 255.0).
...
# layer), its output shape (None means the batch size can be anything), and its number of
# parameters. Note that Dense layers often have a lot of parameters. This gives the
# model quite a lot of flexibility to fit the training data, but it also means that the model runs the
